...
func (c *TCache) BLPopInt(key string, timeout int) (int, error) {
	return Int(c.BLPop(key, timeout))
}

// BLPopInt64 BLPop的工具方法，元素类型为int64时
func (c *TCache) BLPopInt64(key string, timeout int) (int64, error) {
	return Int64(c.BLPop(key, timeout))
}

// BLPopString BLPop的工具方法，元素类型为string时
func (c *TCache) BLPopString(key string, timeout int) (string, error) {
	return String(c.BLPop(key, timeout))
}

// BLPopBool BLPop的工具方法，元素类型为bool时
func (c *TCache) BLPopBool(key string, timeout int) (bool, error) {
	return Bool(c.BLPop(key, timeout))
}

// BLPopObject BLPop的工具方法，元素类型为object时
func (c *TCache) BLPopObject(key string, timeout int, val interface{}) error {
...
// 超时参数 timeout 接受一个以秒为单位的数字作为值。超时参数设为 0 表示阻塞时间可以无限期延长(block indefinitely) 。
func (c *TCache) BRPop(key string, timeout int) (interface{}, error) {
...
// RPush 将一个值插入到列表尾部
func (c *TCache) RPush(key string, member interface{}) error {
	value, err := c.encode(member)
	if err != nil {
		return err
	}
...
// 返回值：被移除元素的数量。
func (c *TCache) LRem(key string, count int, member interface{}) (int, error) {
	return Int(c.Do("LREM", c.getKey(key), count, member))
}

// LLen 获取列表的长度
func (c *TCache) LLen(key string) (int64, error) {
...
func (c *TCache) LRange(key string, start, end int) (interface{}, error) {
...
func (c *TCache) ZRange(key string, from, to int64) (map[string]int64, error) {
...
// 以 0 表示有序集第一个成员，以 1 表示有序集第二个成员，以此类推。或 以 -1 表示最后一个成员， -2 表示倒数第二个成员，以此类推。
func (c *TCache) ZRevrange(key string, from, to int64) (map[string]int64, error) {
	return redis.Int64Map(c.Do("ZREVRANGE", c.getKey(key), from, to, "WITHSCORES"))
}

// ZRangeByScore 返回有序集合中指定分数区间的成员列表。有序集成员按分数值递增(从小到大)次序排列。
// 具有相同分数值的成员按字典序来排列
func (c *TCache) ZRangeByScore(key string, from, to, offset int64, count int) (map[string]int64, error) {
...
// 具有相同分数值的成员按字典序来排列
func (c *TCache) ZRevrangeByScore(key string, from, to, offset int64, count int) (map[string]int64, error) {
...
func (c *TCache) Publish(channel, message string) (int, error) {
...
func (c *TCache) Subscribe(onMessage func(channel string, data []byte) error, channels ...string) error {
	conn := c.pool.Get()
...
func (c *TCache) GeoAdd(key string, longitude, latitude float64, member string) error {
	_, err := redis.Int(c.Do("GEOADD", c.getKey(key), longitude, latitude, member))
	return err
}

// GeoPos 从键里面返回所有给定位置元素的位置（经度和纬度）。
func (c *TCache) GeoPos(key string, members ...interface{}) ([]*[2]float64, error) {
	args := redis.Args{}
...
func (c *TCache) GeoDist(key string, member1, member2, unit string) (float64, error) {
	_, err := redis.Float64(c.Do("GEODIST", c.getKey(key), member1, member2, unit))
	return 0, err
}

// GeoRadius 以给定的经纬度为中心， 返回键包含的位置元素当中， 与中心的距离不超过给定最大距离的所有位置元素。
func (c *TCache) GeoRadius(key string, longitude, latitude, radius float64, unit string, options GeoOptions) ([]*GeoResult, error) {
	args := redis.Args{}
...
func (c *TCache) GeoRadiusByMember(key string, member string, radius float64, unit string, options GeoOptions) ([]*GeoResult, error) {
	args := redis.Args{}
...
func (c *TCache) GeoHash(key string, members ...interface{}) ([]string, error) {
	args := redis.Args{}
	args = args.Add(c.getKey(key))
	args = args.Add(members...)
